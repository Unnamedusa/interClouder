...
function AgeGate({ onPass }) {
  const [step, setStep] = useState(0); const [idxs, setIdxs] = useState([]); const [cur, setCur] = useState(0);
  const [score, setScore] = useState(0); const [time, setTime] = useState(CKIDS.timeLimit);
  const [fails, setFails] = useState(0); const [total, setTotal] = useState(CKIDS.baseCount);
  const start = () => { const pool = [...Array(CKIDS.questions.length).keys()]; for (let i = pool.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [pool[i], pool[j]] = [pool[j], pool[i]]; } setIdxs(pool.slice(0, Math.min(total, pool.length))); setCur(0); setScore(0); setTime(CKIDS.timeLimit); setStep(1); };
  useEffect(() => { if (step !== 1) return; const t = setInterval(() => setTime(p => { if (p <= 1) { clearInterval(t); setFails(f => f + 1); setTotal(q => Math.min(q + CKIDS.penalty, CKIDS.maxCount)); setStep(3); return 0; } return p - 1; }), 1000); return () => clearInterval(t); }, [step]);
  const answer = (i) => { const q = CKIDS.questions[idxs[cur]]; const ok = i === q.a; const ns = ok ? score + 1 : score; if (cur + 1 >= idxs.length) { if (ns >= Math.ceil(idxs.length * 0.8)) { setStep(2); setTimeout(onPass, 1000); } else { setFails(f => f + 1); setTotal(q => Math.min(q + CKIDS.penalty, CKIDS.maxCount)); setStep(3); } } else { setCur(cur + 1); setScore(ns); } };
  if (step === 0) return <main className="age-gate" role="main"><div style={{ width: 400, padding: 30, borderRadius: 20, background: 'var(--bg2)', border: '1px solid var(--bdr)', textAlign: 'center' }}><div style={{ fontSize: 36 }}>🔐</div><h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--acl)', margin: '8px 0' }}>Age Verification</h1><p style={{ fontSize: 12, color: 'var(--txm)', marginBottom: 14 }}>Answer {total} questions to verify 18+.</p><div className="card" style={{ textAlign: 'left', marginBottom: 14, fontSize: 11, color: 'var(--txm)', lineHeight: 1.7 }}>✓ {total} questions · {CKIDS.timeLimit}s timer · 80% to pass{fails > 0 && <div style={{ color: 'var(--wrn)' }}>Failed {fails}x — +{CKIDS.penalty} penalty</div>}</div><button className="btn bp" onClick={start} style={{ width: '100%', padding: 12 }}>Start Verification</button></div></main>;
  if (step === 1) { const q = CKIDS.questions[idxs[cur]]; return <main className="age-gate" role="main"><div style={{ width: 440, padding: 26, borderRadius: 20, background: 'var(--bg2)', border: '1px solid var(--bdr)' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}><span style={{ fontSize: 11, color: 'var(--txm)' }}>Q {cur + 1}/{idxs.length}</span><span style={{ fontSize: 12, fontWeight: 700, color: time < 15 ? 'var(--err)' : 'var(--wrn)', fontFamily: 'monospace' }}>{time}s</span></div><div style={{ display: 'flex', gap: 3, marginBottom: 12 }}>{idxs.map((_, i) => <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i < cur ? 'var(--ok)' : i === cur ? 'var(--acc)' : 'var(--bdr)' }} />)}</div><h2 style={{ fontSize: 14, fontWeight: 700, marginBottom: 14, lineHeight: 1.5 }}>{q.q}</h2><div role="radiogroup">{q.o.map((o, i) => <button key={i} className="btn bs" onClick={() => answer(i)} style={{ width: '100%', textAlign: 'left', padding: 12, marginBottom: 4 }}>{o}</button>)}</div></div></main>; }
  if (step === 2) return <main className="age-gate"><div style={{ textAlign: 'center', padding: 40 }}><div style={{ fontSize: 48 }}>✅</div><h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--ok)', marginTop: 8 }}>Verified!</h2></div></main>;
  return <main className="age-gate"><div style={{ width: 360, padding: 30, borderRadius: 20, background: 'var(--bg2)', border: '1px solid var(--bdr)', textAlign: 'center' }}><div style={{ fontSize: 36 }}>❌</div><h2 style={{ fontSize: 16, fontWeight: 800, color: 'var(--err)' }}>Failed</h2><p style={{ fontSize: 12, color: 'var(--txm)', margin: '8px 0 14px' }}>Next: {total} questions</p><button className="btn bp" onClick={start} style={{ width: '100%' }}>Try Again</button></div></main>;
}

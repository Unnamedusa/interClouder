/* ═══ interClouder v4.5 — Main App + Session Persistence ═══ */
...
function App(){
  ...
  const logout=()=>{SS.c();setUser(null);setSrvs([]);setAS(null);setACh(null);setTestUsers([])};
  ...
  const editUser=(id,ch)=>{
    ...
    if(profUser&&profUser.id===id)setProfUser(p=>({...p,...ch}));
  };

  const delUser=id=>{
    setTestUsers(p=>p.filter(u=>u.id!==id&&u.username!==id));
    setSrvs(p=>p.map(s=>({...s,members:(s.members||[]).filter(m=>m!==id)})));
  };
  ...
  const purchase=tier=>{
    ...
    setUser(p=>{
      ...
      return{...p,premium:tier,badges,animAvatar:P[tier]?.unlocks?.animAvatar||false,animBanner:P[tier]?.unlocks?.animBanner||false};
    });
  ...
  return(
    <div className="app">
      ...
      <div className="sbar">
        ...
        <div style={{flex:1}}/>
        {[["plugins","🧩"],["admin","👑"],["moderation","🛡"],["settings","⚙"]].map(([v2,i])=>(
          <div key={v2} className={`si ${view===v2?"on":""}`} onClick={()=>setView(v2)} style={{background:view===v2?"var(--bga)":"transparent",border:"none",fontSize:15}}>{i}</div>
        ))}
      </div>
      ...
      <div className="main">
          ...
          <div style={{flex:1,display:"flex",overflow:"hidden"}}>
            ...
            {showMem&&<div className="mp">
              ...
              {srvMem.length===0&&<div style={{padding:10,textAlign:"center"}}>
                <p style={{fontSize:10,color:"var(--txg)"}}>No other members</p>
                <p style={{fontSize:9,color:"var(--txf)",marginTop:4}}>CEO Panel → Users → Create Test Account</p>
              </div>}
            </div>}
          </div>
        </React.Fragment>}
        ...
        {view==="settings"&&<SetPanel theme={theme} setTheme={setTheme} user={user} notify={nf} onPay={t=>setPayTier(t)} onLogout={logout} onEditUser={editUser}/>}
      </div>
...
root.render(<App/>);
console.log("%c⬡ interClouder v4.5 — Session Active","color:#A855F7;font-weight:bold;font-size:14px");
